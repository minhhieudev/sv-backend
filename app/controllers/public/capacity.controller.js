...
exports.all = (req, res) => {
    Capacity.find({}).sort({ created: -1 }).exec((err, info) => {
        if (err) {
...
exports.detail = (req, res) => {
    Capacity.findById(req.params.id).exec((err, info) => {
        if (err) {
        ...
        if (info) {
            ...
            Capacity.findOne({
                ...
                if (info) {
                    ...
                    capacity.save((err, capacity) => {
                        if (err) {
                            res.status(500).json({ code: 'error', msg: err });
                            return;
                        }
                        res.status(200).json({ code: 'success', msg: "Thêm thành công", data: capacity });
                    });
                }
            });
        }
        ...
        if (info && info._id != req.params.id) {
            ...
            Capacity.findOne({
                ...
                if (info && info._id != req.params.id) {
                    ...
                    Capacity.findOneAndUpdate(filter, upsertData, { new: false, upsert: true }, (err) => {
                        if (err) {
                            res.status(400).json({ code: 'error', msg: err.message });
                        } else {
...
exports.delete = (req, res) => {
    Capacity.findById(req.params.id).exec((err, info) => {
        ...
        if (info) {
            Capacity.findByIdAndDelete(req.params.id, function(err, info) {
                if (err) {
                    res.status(400).json({ code: 'error', msg: err.message });
                } else {
