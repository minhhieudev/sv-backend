...
exports.all = (req, res) => {
    Vintage.find({}).sort({ created: -1 }).exec((err, info) => {
        if (err) {
...
exports.detail = (req, res) => {
    Vintage.findById(req.params.id).exec((err, info) => {
        if (err) {
        ...
        if (info) {
            ...
            Vintage.findOne({
                ...
                if (info) {
                    ...
                    vintage.save((err, vintage) => {
                        if (err) {
                            res.status(500).json({ code: 'error', msg: err });
                            return;
                        }
                        res.status(200).json({ code: 'success', msg: "Thêm thành công", data: vintage });
                    });
                }
            });
        }
        ...
        if (info && info._id != req.params.id) {
            ...
            Vintage.findOne({
                ...
                if (info && info._id != req.params.id) {
                    ...
                    Vintage.findOneAndUpdate(filter, upsertData, { new: false, upsert: true }, (err) => {
                        if (err) {
                            res.status(400).json({ code: 'error', msg: err.message });
                        } else {
...
exports.delete = (req, res) => {
    Vintage.findById(req.params.id).exec((err, info) => {
        ...
        if (info) {
            Vintage.findByIdAndDelete(req.params.id, function(err, info) {
                if (err) {
                    res.status(400).json({ code: 'error', msg: err.message });
                } else {
