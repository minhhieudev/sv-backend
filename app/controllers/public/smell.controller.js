...
exports.all = (req, res) => {
    Smell.find({}).sort({ created: -1 }).exec((err, info) => {
        if (err) {
...
exports.detail = (req, res) => {
    Smell.findById(req.params.id).exec((err, info) => {
        if (err) {
        ...
        if (info) {
            ...
            Smell.findOne({
                ...
                if (info) {
                    ...
                    smell.save((err, smell) => {
                        if (err) {
                            res.status(500).json({ code: 'error', msg: err });
                            return;
                        }
                        res.status(200).json({ code: 'success', msg: "Thêm thành công", data: smell });
                    });
                }
            });
        }
        ...
        if (info && info._id != req.params.id) {
            ...
            Smell.findOne({
                ...
                if (info && info._id != req.params.id) {
                    ...
                    Smell.findOneAndUpdate(filter, upsertData, { new: false, upsert: true }, (err) => {
                        if (err) {
                            res.status(400).json({ code: 'error', msg: err.message });
                        } else {
...
exports.delete = (req, res) => {
    Smell.findById(req.params.id).exec((err, info) => {
        ...
        if (info) {
            Smell.findByIdAndDelete(req.params.id, function(err, info) {
                if (err) {
                    res.status(400).json({ code: 'error', msg: err.message });
                } else {
