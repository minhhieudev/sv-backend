...
exports.all = (req, res) => {
    Post.find({}, { __v: 0, category: 0, description: 0, comments: 0 }).sort({ created: -1 }).exec((err, posts) => {
        if (err) {
...
exports.add = (req, res, next) => {
    ...
    post.save((err, post) => {
        ...
        res.status(200).json({ code: 'success', msg: "Thêm bài viết thành công", data: { _id: post._id } });
    });
};
